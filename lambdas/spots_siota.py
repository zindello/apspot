...
def lambda_handler(event, context):
    ...
    if '[]' in apiresponse.text or apiresponse.status_code != 200:
        spots.append('No current SIOTA spots in parksnpeaks.org')
    else:
        logging.info('Got SPOTS now need to process just SIOTA spots')
        siotaspots = [x for x in json.loads(apiresponse.text) if x['actClass'] == 'SiOTA']
        if "[]" in json.dumps(siotaspots):
            spots.append('No current SIOTA spots in parksnpeaks.org')
        else:
            if mode in ["SSB","CW","AM", "FM","DATA","PSK","RTTY"]:
                filteredspots = [x for x in siotaspots if x['actMode'] == mode]
                siotaspots = filteredspots
                if len(filteredspots) == 0:
                    spots.append("No current " + mode + " SiOTA spots in parksnpeaks.org")                
            logging.debug(str(siotaspots))
            logging.info('Got valid response from PNP, now to compile and send response')
            for index, spot in zip(range(int(numSpots)), siotaspots):
